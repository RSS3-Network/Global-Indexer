...
func (c *client) FindStakeTransactionsByUser(ctx context.Context, address common.Address) ([]*schema.StakeTransaction, error) {
	...
	if err := c.database.WithContext(ctx).Distinct("*").Where(`"user" = ?`, address.String(), address.String()).Find(&rows).Error; err != nil && !errors.Is(err, gorm.ErrRecordNotFound) {
		return nil, fmt.Errorf("find stake transactions: %w", err)
	}
...
func (c *client) FindStakeTransactionsByNode(ctx context.Context, address common.Address) ([]*schema.StakeTransaction, error) {
	...
	if err := c.database.WithContext(ctx).Distinct("*").Where(`"node" = ?`, address.String(), address.String()).Find(&rows).Error; err != nil && !errors.Is(err, gorm.ErrRecordNotFound) {
		return nil, fmt.Errorf("find stake transactions: %w", err)
	}
