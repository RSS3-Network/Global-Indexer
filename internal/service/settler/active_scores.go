...
func calculateActiveScores(nodes []*schema.Node, recentStakers map[common.Address]*schema.StakeRecentCount, activeScores *config.ActiveScores) ([]*big.Float, error) {
	...
	// Fixme: Public Good nodes do not receive rewards, and this calculation will reduce their active scores.
	...
	aScores := computeActiveScores(nodes, recentStakers, stakingPoolSizes, totalActiveStake, totalStake, activeScores)
	...
	for i, activeScore := range aScores {
