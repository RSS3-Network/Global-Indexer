...
var (
	specialRewards = config.SpecialRewards{
		...
		Rewards:               12328,
		RewardsCeiling:        1000,
		RewardsRatioActive:    0.5,
		RewardsRatioOperation: 0.5,
	}
)
...
func TestCalculateOperationRewards(t *testing.T) {
	...
	correctRewards := [][]string{
		{
			...
			"1000000000000000000000",
			"647000000000000000000",
			"584000000000000000000",
			"398000000000000000000",
			"561000000000000000000",
			"569000000000000000000",
			"614000000000000000000",
			"1000000000000000000000",
	...
	tests := []struct {
		...
		// Mock Nodes
		// [Pool size, Recent Stackers, Recent Stake, Operation Pool Size]
		{
			...
			recentStackers: map[common.Address]*schema.StakeRecentCount{
				...
				common.Address{12}: {StakerCount: 0, StakeValue: decimal.NewFromInt(0)},
			},
