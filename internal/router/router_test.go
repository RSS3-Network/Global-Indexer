...
func TestDistributeRequestWithValidData(t *testing.T) {
	...
	nodeMap := map[common.Address]string{
		common.HexToAddress("0x123"): "http://localhost:8070",
		common.HexToAddress("0x234"): "http://localhost:8080",
		common.HexToAddress("0x567"): "http://localhost:8090",
	}

	mockClient.On("Fetch", mock.Anything, "http://localhost:8080").Return(io.NopCloser(bytes.NewBufferString(nullData)), nil)
...
func TestDistributeRequestWithNodeError(t *testing.T) {
	...
	mockClient.On("Fetch", mock.Anything, "http://localhost:8070").Return(io.NopCloser(bytes.NewBufferString(errResponse)), nil)
	mockClient.On("Fetch", mock.Anything, "http://localhost:8080").Return(io.NopCloser(bytes.NewBufferString(errResponse)), nil)
	mockClient.On("Fetch", mock.Anything, "http://localhost:8090").Return(io.NopCloser(bytes.NewBufferString(errResponse)), nil)
...
func TestDistributeRequestWithError(t *testing.T) {
	...
	mockClient.On("Fetch", mock.Anything, "http://localhost:8070").Return(io.NopCloser(bytes.NewBufferString(errResponse)), errors.New("error 8070"))
